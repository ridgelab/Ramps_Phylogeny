...
def makeArgParser():
    ...
    parser.add_argument('-b', '--outputTAIfile', type=str, help='(output) tAI file calculated from the codon adaptiveness values')
    parser.add_argument('-g', '--outputMeanEfficiencies',type=str, help='(output) file of the harmonic mean of the ramp,after portion, entire sequence, and difference between ramp and after.')
...
def outputRampSeqs(rampSeqs,args):
    ...
    if args.outputMeanEfficiencies:
        meanFile = open(args.outputMeanEfficiencies, 'w')
        meanFile.write("gene,ramp_mean,after_mean,combined_mean,difference_after_and_ramp,ramp_length,ramp_percentage\n")
    ...
    for line in rampSeqs:
        ...
        if args.outputMeanEfficiencies and not line[0].startswith('None'):
            header = line[0].split("\n")[0]
            ramp = line[0].split("\n")[1].strip()
            after = line[1].split("\n")[1].strip()
            complete = ramp + after
            rampSpeeds = calcSeqSpeed(tuple([header,ramp]))[header]
            rampSpeeds = rampSpeeds[1:]
            rampMean = hmean(rampSpeeds)
            afterSpeeds = calcSeqSpeed(tuple([header,after]))[header]
            afterSpeeds = afterSpeeds[:len(afterSpeeds)-1]
            afterMean = hmean(afterSpeeds)
            completeSpeeds = calcSeqSpeed(tuple([header,complete]))[header]
            completeSpeeds = completeSpeeds[1:len(completeSpeeds)-1]
            completeMean = hmean(completeSpeeds)
            difference = afterMean - rampMean
            #print("Ramp: ", ramp)
            #print("Speeds: ", rampSpeeds)
            #print("After: ", after)
            #print("Afterspeeds: ", afterSpeeds)
            meanFile.write(header[1:] + "," + str(round(rampMean,5)) + "," + str(round(afterMean,5)) + "," + str(round(completeMean,5)) + "," + str(round(difference,5)) + "," + str(len(ramp)) + "," + str(round(float(len(ramp))/len(complete) * 100,5))+ "\n")
...
def writeTAIfile(codonToSpeed):
	import csv
	csv_file = args.outputTAIfile
	try:
		with open(csv_file, 'w') as f:
			for key in codonToSpeed.keys():
				f.write("%s,%s\n"%(key,codonToSpeed[key]))
	except IOError:
		print("I/O error Invalid tAI output file path.")
...
if __name__ == '__main__':
    ...
    if args.outputTAIfile:
        writeTAIfile(codonToSpeed)
    if args.verbose:
